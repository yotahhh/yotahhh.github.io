...
function animate() {
    ...
    lines.forEach(line => {
        ...
        if (line.x < 0 || line.x > canvas.width || line.y < 0 || line.y > canvas.height) {
            ...
            line.vx = (Math.random() - 0.5) * 0.035; // Slower animation
            line.vy = (Math.random() - 0.5) * 0.035; // Slower animation
            line.opacity = 0; // Reset opacity when line moves out of canvas
        }
        if (line.opacity < line.maxOpacity) {
            line.opacity += 0.001; // Increase opacity very gradually
        }
    });
...
function init() {
    for (let i = 0; i < 100; i++) {
        lines.push({
            ...
            vx: (Math.random() - 0.5) * 0.035, // Slower animation
            vy: (Math.random() - 0.5) * 0.035,  // Slower animation
            opacity: 0, // Initial opacity is 0
            maxOpacity: Math.random() * 0.42 // Maximum opacity
        });
    }
    animate();
}
